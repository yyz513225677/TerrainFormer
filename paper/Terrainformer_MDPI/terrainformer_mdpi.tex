...
We use $K{=}5$ action chunks aggregated at inference by TemporalEnsemble with exponential-decay weighting ($\lambda{=}0.9$). The choice of $K$ trades two effects:
\begin{itemize}
...
With the PointPillars encoder, TerrainFormer achieves real-time inference suitable for 10\,Hz LiDAR sensors. We characterise real-time performance along four dimensions: per-component latency, end-to-end throughput, latency stability (jitter), and host-resource utilisation.
