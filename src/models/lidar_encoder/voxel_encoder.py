...
class SparseConv3D(nn.Module):
    ...
    def forward(self, features: torch.Tensor, 
        ...
        # Create dense tensor (use same dtype as features for AMP compatibility)
        dense = torch.zeros(batch_size, C, D, H, W, device=device, dtype=features.dtype)
        ...
        if len(out_features_list) > 0:
            ...
            # Use same dtype as dense_out for AMP compatibility
            out_features = torch.zeros(0, self.out_channels, device=device, dtype=dense_out.dtype)
            out_coords = torch.zeros(0, 4, device=device, dtype=torch.long)
...
class VoxelEncoder(nn.Module):
    ...
    def voxelize(self, points: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # Compute voxel indices (use same dtype as points for AMP compatibility)
        points_xyz = points[:, :3]

        voxel_idx = ((points_xyz - torch.tensor(self.point_cloud_range[:3], device=device, dtype=points.dtype))
                     / torch.tensor(self.voxel_size, device=device, dtype=points.dtype)).long()
        ...
        # Initialize outputs (use same dtype as points for AMP compatibility)
        voxel_features = torch.zeros(
            num_voxels, self.max_points_per_voxel, C, device=device, dtype=points.dtype
        )
