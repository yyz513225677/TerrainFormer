...
class BEVProjection(nn.Module):
    ...
    def forward(self, points: torch.Tensor, 
        ...
        # Initialize BEV (use same dtype as features for AMP compatibility)
        bev = torch.zeros(B, self.out_channels, self.height, self.width, device=device, dtype=dtype)
...
class PointPillarProjection(nn.Module):
    ...
    def forward(self, points: torch.Tensor) -> torch.Tensor:
        ...
        bev = torch.zeros(B, self.pillar_channels, self.height, self.width, device=device, dtype=dtype)
        ...
        for b in range(B):
            ...
            # Use same dtype as input points for AMP compatibility
            pillar_x_center = torch.zeros(num_pillars, device=device, dtype=dtype)
            pillar_y_center = torch.zeros(num_pillars, device=device, dtype=dtype)
            pillar_z_center = torch.zeros(num_pillars, device=device, dtype=dtype)
            pillar_counts = torch.zeros(num_pillars, device=device, dtype=dtype)
            ...
            # Max pooling per pillar using segment_reduce approach (gradient-safe)
            # Use same dtype as point_features for compatibility with AMP
            inverse_indices_expanded = inverse_indices.unsqueeze(1).expand(-1, self.pillar_channels)

            # Initialize with zeros and use scatter_reduce with include_self=False
            pillar_features = torch.zeros(
                num_pillars, self.pillar_channels, device=device, dtype=point_features.dtype
            )
            pillar_features = pillar_features.scatter_reduce(
                0, inverse_indices_expanded, point_features, reduce='amax', include_self=False
            )

            # Replace -inf with 0 for empty positions
            pillar_features = pillar_features.clamp(min=0)

            # Scatter to BEV (ensure dtype matches for AMP compatibility)
            bev[b, :, pillar_row, pillar_col] = pillar_features.T.to(bev.dtype)
