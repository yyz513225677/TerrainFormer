...
Combines PointPillars LiDAR encoder, world model, and decision transformer
into a single end-to-end architecture for real-time off-road navigation.
"""
...
class TerrainFormer(nn.Module):
    ...
    Uses PointPillars encoder for real-time BEV projection (~5ms latency),
    enabling end-to-end inference at >30 FPS on modern GPUs.
    """
    ...
    def __init__(self,
        ...
        self.decision_transformer = DecisionTransformer(
            ...
            num_latent_tokens=num_latent_tokens,
            chunk_size=decision_chunk_size,
        )
    ...
    def forward(self,
        ...
        if 'action_chunk_logits' in decision_outputs:
            outputs['action_chunk_logits'] = decision_outputs['action_chunk_logits']
...
def test_terrainformer():
    print("Testing TerrainFormer with PointPillars encoder...")

    model = TerrainFormer(
        lidar_in_channels=4, bev_size=256, num_actions=12
    )
    ...
    print(f"Traversability: {outputs['world_traversability'].shape}")
